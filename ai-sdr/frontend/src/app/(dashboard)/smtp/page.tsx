...
import { Mail, Plus, Settings, TestTube, RefreshCw, AlertTriangle, CheckCircle, XCircle } from "lucide-react"
...
export default function SMTPPage() {
  ...
  useEffect(() => {
    fetchConfigs()
    api.get("/smtp/dns-guide").then(setDnsGuide).catch(() => {})
  }, [])
      ...
      {dnsGuide && (
        <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
          <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
            <Settings size={18} /> DNS Configuration Guide
          </h2>
          <p className="text-gray-400 text-sm mb-4">
            Add these DNS records to your Hostinger DNS panel to ensure deliverability.
          </p>

          <div className="space-y-4">
            {Object.entries(dnsGuide).map(([key, section]: [string, any]) => (
              <div key={key} className="border border-gray-700 rounded p-4">
                <h3 className="text-white font-medium capitalize mb-1">{key}</h3>
